...
describe("OIDCService", () => {
  ...
  it('should delete all sessions for the user', async () => {
    ...
    const deleteAllResultsMock = jest.spyOn(OIDCService, 'deleteAllResults').mockResolvedValue();
  ...
  it('should not throw an error if no sessions are found', async () => {
    ...
    await expect(OIDCService.deleteAllSessions(userId)).resolves.not.toThrow();
  });
  });
