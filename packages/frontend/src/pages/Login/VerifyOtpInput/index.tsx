...
const OtpResendSection: FC<{ handleResendOtp: () => void; timer: number }> = ({ handleResendOtp, timer }) => (
...
const VerifyOtpInput: FC<IVerifyOtpInput> = React.memo(({ email, type, control }) => {
  const { timer, resetTimer } = useTimer();
  const { feedbackAxiosError } = useFeedback();

  useEffect(() => {
    if ([SMS_MFA, EMAIL_MFA, EMAIL_VERIFICATION].includes(type)) handleResendOtp();
  ...
  return (
    <Grid container direction="column" alignItems="center" spacing={4}>
      <Grid item>
        <Typography align="center"><OtpMessage type={type}/></Typography>
      </Grid>

      {type !== APP_MFA && <OtpResendSection handleResendOtp={handleResendOtp} timer={timer} />}
